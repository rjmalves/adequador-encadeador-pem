...
from idecomp.decomp.modelos.dadger import ACCOTVOL, ACJUSMED, ACCOTVAZ, ACPROESP, ACPERHID, ACNCHAVE, ACTIPERH, ACVERTJU, ACNPOSNW, ACVAZMIN
...
df_usinas_cmont_cfuga = pd.read_csv(join(".\gtdp","usinas_cmontcfuga.csv"), sep=";")
df_nposnw = pd.read_csv(join(".\gtdp","ac_nposnw.csv"), sep=";")
df_nposnw = df_nposnw.loc[~df_nposnw["usinas_nposnw"].str.contains("&")]
df_vertju = pd.read_csv(join(".\gtdp","ac_vertju.csv"), sep=";")

usinas_cmont_cfuga = df_usinas_cmont_cfuga["usinas_cmontcfuga"].tolist()
    ...
    def adiciona_ac_nposnw(dadger: Dadger, u: int, p: int):
        ...
        if usina is not None:
            ...
            if reg is None: 
                ...
                posicao = dadger.lista_registros(ACVAZMIN)[-1] # Escolhe  posicao para colocar os novos registros
                ac_novo = ACNPOSNW()
                ac_novo.uhe = u
                ac_novo.posto = p
