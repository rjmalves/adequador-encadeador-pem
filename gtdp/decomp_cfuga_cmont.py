...
df_cmont_historico = pd.read_csv(join(".\gtdp","cfuga_cmont_historico.csv"), sep=";")
df_cmont_historico = df_cmont_historico.loc[~df_cmont_historico["usina"].str.contains("&")]
...
df_cmont_cfuga = pd.read_csv(join(".\gtdp","cfuga_cmont.csv"), sep=";")
...
def adequa_cfuga_cmont(diretorio: str, arquivo: str):
    ...
    dataini = datetime.date(day=dadger.dt.dia,month=dadger.dt.mes,year=dadger.dt.ano) # data de inicio do 1º estágio
    anodeck = (dataini + datetime.timedelta(days=7)).year # ano do PMO

    datasdeck = [dataini]
    for e in range(1,n_est,1):
        datasdeck = datasdeck + [dataini + datetime.timedelta(days=7*e)] # datas iniciais de todos os estágios
    ...
    def cria_cotvol(dadger: Dadger,usina,mes,semana,ano,ordem,cmont):
        ...
        posicao = dadger.lista_registros(ACVAZMIN)[-1] # Escolhe uma posicao para colocar os novos registros
        ac_novo = ACCOTVOL()
        ac_novo.uhe = usina
        ac_novo.mes = mes
    ...
    def altera_jusmed_usina(dadger: Dadger ,usina,meses,anos,inds,cfuga_tucurui):

        for m in [1,0]:
            reg = dadger.ac(uhe = usina, modificacao = ACJUSMED, mes=meses[inds[m]])
    ...
    def altera_cotvol_jusmed_usina(dadger: Dadger,usina,n_est,meses,anos,inds,cmont_usina,cfuga_usina,cmont_antes):
        ...
        for m, e in estagios:
            ...
            if m == 0: # se 1º mês
                periodo = e
                reg_cotvol = dadger.ac(uhe = usina, modificacao = ACCOTVOL,mes=meses[inds[m]],semana=periodo,ordem=1)
                reg_jusmed = dadger.ac(uhe = usina, modificacao = ACJUSMED,mes=meses[inds[m]],semana=periodo)
            else: # se 2º mês
                periodo = 1
                reg_cotvol = dadger.ac(uhe = usina, modificacao = ACCOTVOL,mes=meses[inds[m]],ordem=1)
                reg_jusmed = dadger.ac(uhe = usina, modificacao = ACJUSMED,mes=meses[inds[m]])
            ...
            if cmont is None:
                ...
                for o in [5,4,3,2]:
                    cria_cotvol(dadger,usina,meses[inds[m]],periodo,anos[m],o,0.00)
                cria_cotvol(dadger,usina,meses[inds[m]],periodo,anos[m],1,cmont_usina[inds[m]])
            ...
            if reg_jusmed is not None:
                ...
                cria_jusmed(dadger,usina,meses[inds[m]],periodo,anos[m],cfuga_alterado)
    ...
    for usina in usinas_cmont_cfuga:
        if usina not in usinas_cmont_historico:
            ...
            cfuga = df_cmont_cfuga.loc[df_cmont_cfuga["usina"] == usina,"cfuga"].tolist()
            dadger = altera_jusmed_usina(dadger,usina,meses,anos,inds,cfuga)
        else:
            ...
            df_cmont_historico_uhe = df_cmont_historico.loc[df_cmont_historico["usina"] == str(usina),:]
            ...
            cmont_ciclopassado = df_cmont_historico_uhe.loc[df_cmont_historico_uhe["ano_inicio"]==ano_referencia_comparacao,
                                                            "cmont_historico"].tolist()
            cmont = df_cmont_cfuga.loc[df_cmont_cfuga["usina"] == usina,"cmont"].tolist()
            cfuga = df_cmont_cfuga.loc[df_cmont_cfuga["usina"] == usina,"cfuga"].tolist()
            dadger = altera_cotvol_jusmed_usina(dadger,usina,n_est,meses,anos,inds,cmont,cfuga,cmont_ciclopassado)
