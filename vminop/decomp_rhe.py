...
def ajusta_rhe(diretorio: str, arquivo: str, REEs_RHE: list, vminop: list, tipo_penalidade: list):
    ...
    for  r in REEs_RHE:
        if pmo in [11,12,1]: # se for PMO de nov, dez ou jan
            ...
            if rv0 == "sim": # é primeira semana
                estagio_1 = min(vminop[r][indice_rhe_m0],vminop[r][indice_rhe_m1]) 
            else:
    ...
    def cria_HE(dadger: Dadger,codigo,tipo_limite,limite,estagio,penalidade,tipo_penalidade,posicao_registro):
        he_novo = HE()
    ...
    for (r,e) in list(reversed(rhes_necessarios)):
        ...
        if [r,e] in rhes_existentes: # Já existe RHE para o REE e estágio
            ...
            dadger.he(codigo = codigo, estagio = estagio).limite = vminop_rhe[ind_REE][estagio-1]
            ...
            cria_HE(dadger, id, 2, vminop_rhe[ind_REE][estagio-1], 
                        estagio, penalidade, tipo,
                        posicao)
